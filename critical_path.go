...
func calculateCriticalPath(tasks []Task) []Task {
	...
	for i := range tasks {
		...
		for j := range tasks[i].ChildUniqueIDs {
			_ = gTree.AddEdge(tasks[i].UniqueID, tasks[i].ChildUniqueIDs[j])
		}
	}
	...
	for i := range tasks {
		...
		_ = graph.DFS(g, tasks[i].UniqueID, func(value string) bool {
			CurrentChild = append(CurrentChild, value)
			return false
		})
		fmt.Println("[CURRENTCHILD]", CurrentChild)
		if tasks[i].Successors != nil {
			// Step 1: Forward Pass
			for j := range currentPredID {
				// define ES and EF currentPredID the first task
				...
				if len(tasks[taskIndices[currentPredID[j]]].Predecessors) > 0 {
					for _, predecessor := range tasks[taskIndices[currentPredID[j]]].Predecessors {
						predecessor.ID = tasks[taskIndices[predecessor.UniqueID]].UniqueID
					}
				}
			}
		}
	...
	for i := range tasks {
		...
		for j := len(currentPredID) - 1; j >= 0; j-- {
			...
			if len(tasks[taskIndices[currentPredID[j]]].Successors) == 0 {
				tasks[taskIndices[currentPredID[j]]].LateFinish = longestFinish
				tasks[taskIndices[currentPredID[j]]].LateStart = longestFinish
				// LS := tasks[taskIndices[currentPredID[j]]].LateFinish - tasks[taskIndices[currentPredID[j]]].Duration
			}
		}
	}
	for j := len(tasks) - 1; j >= 0; j-- {
		if len(tasks[j].ChildUniqueIDs) > 0 {
			for _, child := range tasks[j].ChildUniqueIDs {
				...
				fmt.Println("[CHILD]", child, tasks[taskIndices[child]].LateStart, tasks[taskIndices[child]].LateFinish, tasks[taskIndices[child]].Duration)
			}
			fmt.Println("[PARENT]", tasks[j].UniqueID, tasks[j].LateStart, tasks[j].LateFinish, tasks[j].Duration)
		}
	}
	// for i := range tasks {
	// 	if tasks[i].DataType == "header" {
