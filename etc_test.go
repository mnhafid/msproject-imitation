...
func TestEtcConstraint(t *testing.T) {

	t.Run("Test Success ETC with constraint must finish on", func(t *testing.T) {
		ProjectCalendars := []ProjectCalendar{
			{
				ID:   "1",
				Name: "Test",
		...
		TaskCriticalPath := []Task{
			{UniqueID: "1",
				...
				ConstraintDate:  "2024-08-03T08:00",
			},
			{UniqueID: "2",
				Duration:        10,
				Successors:      []Successor{},
		...
		result := []time.Time{
			time.Date(2024, 8, 3, 8, 0, 0, 0, time.UTC),
			time.Date(2024, 8, 3, 8, 0, 0, 0, time.UTC),
			time.Date(2024, 9, 30, 8, 0, 0, 0, time.UTC),
			time.Date(2024, 8, 31, 0, 0, 0, 0, time.UTC),
		}
